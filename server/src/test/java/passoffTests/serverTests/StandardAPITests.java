...
public class StandardAPITests {
    ...
    public void uniqueAuthorizationTokens() throws TestException {
        ...
        assertNotEquals(existingAuth, loginOne.authToken,
                "Authtoken returned by login matched authtoken from prior register");
        assertNotEquals(existingAuth, loginTwo.authToken,
                "Authtoken returned by login matched authtoken from prior register");
        assertNotEquals(loginOne.authToken, loginTwo.authToken,
        ...
        Assertions.assertFalse(
                ...
                "Response gave an error message");
        assertEquals(1, listResult.games.length);
        assertEquals(existingUser.username, listResult.games[0].whiteUsername);
    }
    ...
    public void registerTwice() throws TestException {
        ...
        assertTrue(registerResult.message.toLowerCase(Locale.ROOT).contains("error"),
                "Response missing error message");
        assertNull(registerResult.username, "Response incorrectly contained username");
        assertNull(registerResult.authToken, "Response incorrectly contained authentication string");
    }
    ...
    public void failRegister() throws TestException {
        ...
        assertEquals(HttpURLConnection.HTTP_BAD_REQUEST, serverFacade.getStatusCode(),
    ...
    public void goodWatch() throws TestException {
        ...
        assertEquals(HttpURLConnection.HTTP_OK, serverFacade.getStatusCode(),
                "Server response code was not 200 OK");
        assertEquals(1, listResult.games.length, "List Games returned an incorrect number of games");
        assertNull(listResult.games[0].whiteUsername, "Player present on a game that no player joined");
        assertNull(listResult.games[0].blackUsername, "Player present on a game that no player joined");
    }
